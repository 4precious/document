...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item SQL\\
        SQL stands for Structured Query Language. It is used to communicate with data within a database management system. SQL was used in order to store, retrieve, manage and manipulate data: users’ information, contents of questions and answers, etc. Relational databases store information in tabular form, and rows and columns represent different relationships between different data properties and data values. SQL can also be used to maintain and optimize database performance. In addition, Structural Query Language (SQL) is a widely used query language frequently used in all types of applications. Data analysts and developers learn and use SQL because it integrates well with different programming languages.\\
    \end{enumerate}
    \newpage
    \item Software
...
\subsection{Sign up}
    ...
    \item Main page \hfill \break
            ...
            \begin{enumerate}
                ...
                \begin{figure}[H]
                 \centering
                 \includegraphics[scale=0.18]{qa.jpg}
                ...
                \begin{figure}[H]
                 \centering
                 \includegraphics[scale=0.28]{cal.png}
                 \end{figure}
            \end{enumerate}
            ...
            \begin{figure}[H]
                 \centering
                 \includegraphics[scale=0.28]{solu.png}
...
\begin{enumerate}
    ...
    \begin{flushleft}
        ...
        \midrule
        /frontend/aurum/assets/icons & \makecell[l]{Babychick.svg\\Chicken.svg\\calendar.svg\\leaves.svg\\loveletter.svg\\studycgild.svg\\oven.svg\\TV.svg\\arrow.svg\\blueberry.svg\\fish.svg\\walnut.svg\\vector412.svg}\\
        \midrule
        ...
        \bottomrule
        \makecell[l]{/frontend/aurum/src/utils} & \makecell[l]{const.ts\\fetchWithAuthentication.ts\\getAnswer.ts\\getLatestQuestion.ts\\getQuestion.ts\\getRepresentEmotion.ts\\getTodayInString.ts\\isLogin.ts\\loginWithCredentials.ts\\uploadQuestion.ts\\uploadAnswer.ts}\\
        \end{supertabular}
    \end{flushleft}
    \vspace{0.5cm}
    \item back-end
    ...
    \item Front-end
       \begin{enumerate}
           ...
           \begin{enumerate}
               ...
               \newline
               \item Calendar
               \\This calendar component is common to both parents and child users. Pressing the calendar button on the main page takes user to this page. Here, the child's emotional report can be viewed as a monthly view in calendar form. Here, we send the date to the server, get the representative emotion of that day. If the user is curious about the detailed reason, when they see the representative emotion, click that column of the calendar. Then, user can go to the child's answer and emotion analysis results page of that day. Before moving, ask the user one more time if they really want to move to the page through the alert function. If the user presses the future calendar column, the user is notified with the alert function, "The date has not arrived yet."
               \newline
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \includegraphics[scale=0.1]{UseCases/20.sadicon.png}
        \end{figure}
        \item Use Case 2
        \\If parent user register the question, the expression of the child icon changes according to the child's current feelings. At this time, the child's emotions are expressed in six ways: happiness, sadness, anger, anxiety, injury, and embarrassment. The example screen above shows the child icon when she is happy and angry. And the content of the button changes from "Write a Question" to "Check Child Answer."
        \begin{figure}[H]
         \centering
        ...
        \begin{figure}[H]
        \centering
        \includegraphics[scale=0.1]{UseCases/6.postQ2.png}
        \end{figure}
        \item Use Case 5
        \\If the parent user has registered a question, but the child user has not yet registered an answer, a notice appears in the application as shown in the screen above.
        \begin{figure}[H]
        \centering
        \includegraphics[scale=0.1]{UseCases/7.presult.png}
        \includegraphics[scale=0.1]{UseCases/7.presult2.png}
        \end{figure}
        \item Use Case 6
        \\If the child user leaves an answer according to the question left by the parent, parent user can confirm it. And then, the sentimental analysis result of this answer is automatically displayed. Also, at the bottom of the screen, parent user can compare yesterday's child feelings with today's. "Compare yesterday" can only be compared with the representative sentiment that had the highest percentage on that day. If the child's sentiment is the same as yesterday, user can compare how different the proportion of the sentiment is in the total mood.
        \begin{figure}[H]
        \centering
        \includegraphics[scale=0.1]{UseCases/8.category.png}
        \includegraphics[scale=0.1]{UseCases/8.category2.png}
        \end{figure}
        \item Use Case 7
        \\If the child's emotions are negative and parenting coaching is needed for the answer, the parent user goes to the solution page by clicking the solution button. The content of this button changes according to the child's feelings. In the case of the example screen, since the child's representative emotion on the day is 'anxiety', parents can check the coaching for the anxious child. 
        ...
        \includegraphics[scale=0.1]{UseCases/10.lgoven.png}
        \end{figure}
        \item Use Case 8
        \\After the parent user checks the contents of the solution that fits the child's situation, there is a customized home appliance service tab at the bottom of the page. 
        \begin{figure}[H]
        \centering
        ...
        \begin{figure}[H]
        \centering
        \includegraphics[scale=0.1]{UseCases/11.menu.png}
        \includegraphics[scale=0.1]{UseCases/12.recipe.png}
        \end{figure}
        \item Use Case 9
        \\If the parent user clicks on the "Cooking" card in the home appliance solution, they can go to the following page. In this case, The parent users may also browse the recipe for cooking the food. 
        ...
        \includegraphics[scale=0.1]{UseCases/9.solution_today.png}
        \end{figure}
        \newpage
        \item Use Case 10
    ...
    \begin{enumerate}
        ...
        \includegraphics[scale=0.1]{UseCases/13.cmain2.png}
        \end{figure}
        \item Use Case 1 
        \\If question has not yet registered from the parent user, the main page displays the phrase "The question has not arrived yet." Also, the leaves icon, not the love letter icon, is displayed. 
        ...
        \includegraphics[scale=0.1]{UseCases/14.postA2.png}
        \end{figure}
        \item Use Case 3
        \\When the child user completes the registration of the answer corresponding to the parent's question, it goes through the confirmation process through the alert window. When the child user clicks the confirmation button, the child's written answer is sent to the parent.
        \begin{figure}[H]
        \centering
        \includegraphics[scale=0.1]{UseCases/15.cresult.jpg}
        \end{figure}
        \item Use Case 4
        \\Then, the emotion analysis result for the answer is automatically displayed in the application. Through these results, child users can better understand their feelings.
    \end{enumerate}
\end{enumerate}
\vspace{0.5cm}
...
\bibitem{b1} KOSTAT, Statistics Korea (https://kostat.go.kr/portal/korea/index.action)
\bibitem{b2} Ministry of Gender Equality and Family Statistics \\(http://www.mogef.go.kr/io/stt/io\_stt\_s003.do)
\bibitem{b3} Google Colab (https://colab.research.google.com/github/SKTBrain\\/KoBERT/blob/master/scripts/NSMC/\\naver\_review\_classifications\_pytorch\\\_kobert.ipynb\#scrollTo=itIExnuLbSap)
\bibitem{b4} SKT Brain KoBERT (https://github.com/SKTBrain/KoBERT)
\bibitem{b5} Fine-tuning Pre-trained BERT Models\\ (https://nlp.gluon.ai/examples/sentence\_embedding/bert.html)
\end{thebibliography}
