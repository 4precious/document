...
\begin{enumerate}
    ...
    \item Back-end
        ...
        \item Class Component
        \begin{enumerate}
            \item texts/views.py , users/views.py
            ...
            \item texts/models.py, users/models.py
            \\Models.py is responsible for creating models in the django framework. When creating class-type data model in the models.py, django creates a data model in the database through object-relational-mapping (ORM).

            \item texts/serializers.py, users/serializers.py
            \\Serializer serves as an adapter that converts complex objects such as queryset and model instances that we use in Django into json-like forms that we will use in the REST API. Today's API servers typically use JSON encoded requests/responses.
...
\bibitem{b1} Google Colab (https://colab.research.google.com/github/SKTBrain/KoBERT/\\blob/master/scripts/NSMC/naver\_review\_classifications\_pytorch\\\_kobert.ipynb#scrollTo=itIExnuLbSap)
